...
import { claimTrade } from "../wallet/ClaimTrade";
...
function ToClaim({ toClaim, price, userAccount, walletPubKey, connection, provider }: Props) {
  ...
  const handleTrade = (tradeData: any) => {
    if (tradeData && userAccount && walletPubKey && connection && provider) {
      ...
      const tradeResult = claimTrade(passTrade);
      if (!tradeResult) {
  ...
  return (
    <div className="mt-5">    
      {toClaim.length === 0 && <>
        <p>You have no pending orders to claim.</p>
      </>}
      {toClaim.length > 0 && userAccount && (
      <>
        <div className="mb-5">All orders that you your or trading counterpart can claim relating to your trading account</div>
        {toClaim?.map((tradeData) => {
          let isUsers = false;
          let isMaker = false;
          let isTaker = false;
          ...
          if (isUsers) {
                ...
                <div className="flex flex-wrap lg:flex-row mb-2">
                  ...
                  <div className="flex flex-grow">
                    <div className="mr-3 mt-2 flex flex-col w-full items-center justify-center py-4 bg-gray-200 border-[1px] border-gray-200">
                      <div className="text-xs">Profit Expectation</div>
                  ...
                  {isUsers && 
                    <div className="flex flex-grow">
                      <div className="mr-3 mt-2 flex flex-col w-full items-center justify-center py-4 bg-gray-100 border-[1px] border-gray-200">
                        <Button onClick={() => handleTrade(tradeData)} variant="outlined" color="info">Claim</Button>
                      </div>
                    </div>
                  }
                </div>
            ...
            <div>
              <p>You currently do not have any trades that can be closed out and redemmed.</p>
              <p>
